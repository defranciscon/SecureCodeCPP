...
std::string read_file(const std::string& filename)
{
    ...
    // read text from file using ifstream class
    std::ifstream file_stream(filename);

    // read text file line by line
    std::getline(file_stream, file_text, '\0');
...
{
    // Define date/time variables creating data/time objects
    struct tm new_time {};
    __time64_t long_time;
    errno_t err;

    // get the current time as 64-bit integer
    _time64(&long_time);

    // converts to local time
    err = _localtime64_s(&new_time, &long_time);
    
    // Convert date structure elements to strings and
    // pass through ofstream to write data to file
    std::string year = std::to_string(new_time.tm_year + 1900);
    std::string month = std::to_string(new_time.tm_mon + 1);
    std::string day = std::to_string(new_time.tm_mday);
    std::string zero = "0";

    // concatenate 0 in front of numeric month if before October i.e. MM = 10
    if (new_time.tm_mon + 1 < 10) {
        month = zero + month;
    }
    ...
    data_file << student_name << std::endl;
    data_file << year << "-" << month << "-" << day << std::endl;
    data_file << key << std::endl;
