...
// (1). Detect when a numeric overflow or underflow is about to happen.
// (2). Write code to prevent the numeric overflow or underflow from happening.
// (3). Protect your code from underflow and overflow in any of the standard C/C++ data types.
...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        // screens for integer overflow based on mismatches between input/output and wraparound
        // Adapted from rathbhupendra and Sonu Kumar Pandit (2023)
        //https://www.geeksforgeeks.org/check-for-integer-overflow/
        try {
            *screener = result + increment;

            // mismatched output +a , +b : -c 
            if (result > 0 && increment > 0 && *screener < 0)
            {
            ...
            // wrap around of unsigned values
            if (*screener <= result)
            {
                throw std::overflow_error("wraparound overflow detected: ");
            }
            result += increment;
        }
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // screens for integer underflow based on mismatches between input/output and wraparound
        // Adapted from rathbhupendra and Sonu Kumar Pandit (2023)
        //https://www.geeksforgeeks.org/check-for-integer-overflow/
        try {
            *screener = result - decrement;
            
            // mismatched output +a , +b : -c 
            if (*screener < 0 && result > 0 && decrement > 0)
            {
            ...
            if (*screener >= result)
            {
                throw std::underflow_error("wraparound underflow detected: ");
            }
            result -= decrement;
        }
            catch (const std::exception& e) {
...
void test_overflow()
{
    ...
    // add_numbers function embedded into try and catch blocks for detecting overflows
    try {
...
void test_underflow()
{
    ...
    std::cout << +subtract_numbers<T>(start, decrement, steps) << std::endl;

    std::cout << "\tSubtracting Numbers With Underflow (" << +start << ", " << +decrement << ", " << (steps + 1) << ") = ";

    // subtract_numbers function embedded into try and catch blocks for detecting underflows
    try {
        std::cout << +subtract_numbers<T>(start, decrement, steps + 1) << std::endl;
    }
