...
{
  // whitelist input validation to sanitize user input and detect an sql injection
  // Valid input: 
  // (1). alphabetic characters delimited by whitespace,
  // (2). Minimum length = 2; Maximum length = 15.
  // (3). ' and " punctuation characters limited allowance of 0 to 1 occurences
  // https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html

  regex reg_exp("[^A-Z{2,15}+$]\\s*(and|or)\\s*[\\'\"]{0,1}");

  // uses the regex_search function to match a subsequence in the sql object with the regular expression
  // https://www.tutorialspoint.com/cpp_standard_library/cpp_regex_search.htm

  if (regex_search(sql, reg_exp))
  {
      std::cout << endl;
      std::cout << "SQL Injection Detected. Injection Prevented." << std::endl;
      std::cout << "Detected SQL Statement: " << sql.c_str() << endl;
      std::cout << endl;
      return false;
  }
  /*
  ...
  */
  ...
  query = sqlite3_exec(db, sql.c_str(), callback, &records, &error_message);
  if (query != SQLITE_OK)
  {
...
int main()
{
  ...
  if(db != NULL)
  {
    ...
    // display message to confirm database connection is closed
